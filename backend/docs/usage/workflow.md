...
## 快速 Demo：匯入 Gemini 對話
若只想展示新的雙人對話播客功能，可跳過完整 CLI，改用下列快速流程：

1. 在 `gemini-2-podcast/` 依該 repo README 生成 `podcast_script.txt` 與 `final_podcast.wav`（可加 `--language`）。
2. 回到本專案，可選擇：
   - `./run.sh` 選單中選「匯入 Gemini 對話 Demo」，依提示輸入參數（腳本會自動移除 `Speaker A/B:` 標籤並生成 MP3）；或
   - 執行指令：
   ```bash
   python scripts/import_gemini_dialogue.py \
     ...
     --language en
   ```
3. 啟動 `uvicorn` 後，即可透過既有 API / iOS App 播放該章節。若需要字幕，將匯入後的腳本 + 音訊丟進原本的字幕對齊流程即可。
