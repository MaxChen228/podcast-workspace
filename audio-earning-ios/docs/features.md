...
| 新聞牆 / 搜尋 | 「新聞」分頁 → 選分類或輸入關鍵字 | `NewsFeedView`, `NewsArticleRow`, `NewsFeedViewModel` | `NewsService`, `NewsPreferenceStore`; 後端 `/news/*` | 借力 Bing News API，支援分類快取、搜尋、互動事件上報；市場與分類透過 `NewsPreferenceStore` 記憶。 |
